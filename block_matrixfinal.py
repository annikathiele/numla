...
def get_dn():
    """
    Lässt Nutzer Dimension und n wählen
    Parameter
    ------
    None
    Returns
    ...
    ValueError
    """
    print("Wählen Sie zunächst die Dimension")
    while True:
        try:
            d = input("Bitte wählen Sie eine ganze Zahl zwischen 1 und 3: ")
            d = int(d)
            if 1<=d<=3:
                break
            ...
            print("Keine gültige Zahl, probiere es nochmals ...")
    print("Wählen Sie als nächstes ein n")
    while True:
        try:
            n = input("Bitte wählen Sie eine ganze Zahl zwischen 3 und 15: ")
            n = int(n)
            if 3<=n<=15:
                break
            print("Keine gültige Zahl, probiere es nochmals ...")
...
und Sparsity unserer Koeffizientenmatrix bestimmen.")
    ...
    p,l,u = A.get_lu()
    print()
    print("Für d =" , d , "und n =",n, "hat die Koeffizientenmatrix folgende Kondition")
    print(A.get_cond())
    print()
    print("Die LU-Zerlegung sieht wie folgt aus:")
    print("P =",p)
    print("L =", l)
    print("U =", u)
    print()
    print("Und dies ist die Anzahl der Nicht-Null-Einträge der LU-Zerlegung")
    print(A.eval_sparsity_lu())
    print()
    print("Wir nutzen unsere Funktionen um zwei Plots zu erstellen")
...
def graph_cond():# pylint: disable-msg=too-many-locals
    ...
    plt.figure()
    plt.plot(Nlist_one, ylist_one, 'c-',label= 'd=1' )
    plt.plot(Nlist_two, ylist_two, 'm-',label= 'd=2' )
    plt.plot(Nlist_three, ylist_three, 'k-',label= 'd=3' )
    plt.xscale("log")
    plt.yscale("log")
    plt.xlabel("N")
    plt.ylabel("cond")
