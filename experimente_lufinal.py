...
def main():
    ...
    while True:
        try:
            _Kappa = input("Bitte wählen Sie eine ganze Zahl zwischen 1 und 100: ")
            _Kappa = int(_Kappa)
            if 1<=_Kappa<=100:
                break
            ...
            print("Keine gültige Zahl, probiere es nochmals ...")
    print("Es wurde Kappa =", _Kappa, "festgelegt.")
...
def u(x):
    ...
    for counter, value in enumerate(x):
        prod=prod*value* np.sin(_Kappa*np.pi*value)
    return prod
...
def f(x):
    ...
    for count, value in enumerate(x):
        ...
        for c,v in enumerate(x):
            if c!=count:
                prod=prod*float(v)* np.sin(float(_Kappa)*np.pi*float(v))
        prod_one = -prod*value*((_Kappa*np.pi)**2)*np.sin(_Kappa*np.pi*value)
        prod_two = prod*(1+_Kappa*np.pi)*np.cos(_Kappa*np.pi*value)
...
def graph3D():# pylint: disable-msg=too-many-locals
    """
    Diese Funktion erstellt einen Graphen, der die exakte und approximierte
    Lösung des Poissionproblems für n=4 und n=9 ind der Dimension d=2 darstellt.
    Returns
    -------
    None
    """
...
label= 'Approximierte Lösung für n=9' )
...
def vgl_cond():
    """
    Gibt für n=2 bis n=13 die Konditionen der Hilbert und Blockmatritzen der
...
def m_errors():
    ...
    print("Dazu betrachten wir ein bis drei Dimensionen und bis zu 9261 \
Diskretisierungpunkte")
