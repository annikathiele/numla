...
def solve_sor(A, b, x0, eps=1e-8, max_iter=1000, min_red=1e-4, omega=0.48):
    ...
    """
    A_d=A.todense()
    residual = np.linalg.norm(np.add(np.dot(A_d, x0),-b))
    counter=0
    while residual > eps:
        for i in range(sp.csr_matrix(A).shape[0]):
            ...
            for j in range(sp.csr_matrix(A).shape[1]):
                if j != i:
                    sigma += A[i,j] * x0[j]
            x0[i] = (1 - omega) * x0[i] + ((omega / A[i,i]) * (b[i] - sigma))
        new_residual = np.linalg.norm(np.add(np.dot(A_d, x0),-b))
...
def main():
    A=np.matrix([[1,3,4,5],[3,7,2,3],[4,3,7,8],[4,9,5,6]])
    A=sp.csr_matrix(A)
    b=np.array([4,6,3,5])
    #b=sp.csr_matrix(b)
    x0=np.array([1,1,1,2])
