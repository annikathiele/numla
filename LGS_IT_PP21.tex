...
Wir betrachten für die Dimensionen $d\in\{1,2,3\}$ die approximierte Lösung der Poisson-Gleichung durch das SOR-Verfahren für verschiedene Anzahlen an Teilintervallen $n$.   Wir schauen uns den Fehler für feste Abbruchbedingungen an,  entweder wenn $||Ax_i-b||-||Ax_{i+1}-b||<10^{-7}$, $||Ax_i-b||<10^{-8}$ oder wenn die maximale Iterationszahl von $1500$ überschritten ist.  In der folgenden Abbildung,  ist der maximale absolute Fehler der iterierten Lösung in Abhängigkeit von $n$ geplottet.
\subsection{Konvergenzverhalten mit verschiedenen Abbruchbedingungen}
Wir untersuchen das Konvergenzverhalten 
...
Bei dem Poisson Problem betrachten wir 
