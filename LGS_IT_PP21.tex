...
Wir schauen uns zunächst an, wie sich der Fehler im Laufe der Iterationen verhält. Dazu betrachten wir die folgende Abbildung \ref{error}, in der der maximale absolute Fehler der Iterierten für $d\in\{1,2,3\}$ und $27$ Diskretisierungspunkten. In der Abbildung \ref{error2} hingegen ist dieses Konvergenzverhalten für $d\in\{1,2,3\}$ und $144$ Diskretisierungspunkten abgebildet. Wir wählen die Abbruchbedingungen max\_ iter=$5000$, $\varepsilon =10^{-8}$, min\_ red aufgehoben, $\omega =0,6$. WIr heben die Abbruchbedingung min\_ red auf, da der Fehler nicht streng monoton fällt.
